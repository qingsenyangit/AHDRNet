...
template<typename T>
void OutPutData( const string &Filename, T *pData, int row, int col)
{
	fstream txt_file(Filename.c_str(), ios::out);
	if(txt_file.fail() )
	{
	...
	for(int i = 0; i < row; i++)
	{
		for(int j = 0; j < col; j++)
		{
			txt_file<<pData[i*col+j]<<"\t";
		}
		txt_file<<endl;
	}
	cout<<"Saving "<<Filename<<" successfully.."<<endl;
	return;
}
...
{
	Image<T1> foo;
//     cout<<"sigma:"<<sigma<<"\tfsize"<<fsize<<endl;
...
//     cout<<"width"<<image.width()<<endl;
	...
	for(int i=0;i<2*fsize+1;i++)
		gFilter[i]/=sum;
//     OutPutData( "image.txt", image.data(),image.height(),image.width());
	// apply filtering
...
//     cout<<"entering hfilter"<<endl;
	ImageProcessing::hfiltering(pData,pTempBuffer,imWidth,imHeight,nChannels,hfilter,hfsize);
//     cout<<"entering vfilter"<<nElements<< endl;
//         cout<<image.width()<<" "<<image.height()<<imWidth<<" "<<imHeight<<endl;
// 	OutPutData("hfiltering.txt", pTempBuffer, imHeight, imWidth*nChannels);
